...
To isolate pure shot-creation performance from ball-handling errors, this model intentionally excludes turnovers from the scoring-opportunity denominator and standardizes all box-score totals into precise per-game averages using total games played ($D_2$).

### Shot Creation Execution Rating (SCER) Formula:
$$SCER = \left( \frac{\frac{\text{PTS}}{D_2} + 2\left(\frac{\text{AST}}{D_2} \times 0.69\right) + 3\left(\frac{\text{AST}}{D_2} \times 0.31\right) + \left(0.15 \times 2 \times \$S\$20\right)}{\frac{\text{FGA}}{D_2} + 0.44\left(\frac{\text{FTA}}{D_2}\right) + \frac{\text{AST}}{D_2} + 0.15} \right) \times 100$$
...
![Roster Risk vs Reward Matrix](chart.png)
